...
# CSS in base64 kodieren für sichere Übertragung
import base64
css_encoded = base64.b64encode(css_content.encode('utf-8')).decode('utf-8')
...
# CSS auf Server kopieren und dekodieren
child.sendline(f"echo '{css_encoded}' | base64 -d > /tmp/custom-css.txt")
child.expect(['# ', '$ '])
...
# CSS direkt als String übergeben (ohne Pipe)
child.sendline("docker exec nextcloud bash -c \"cd /var/www/html && php occ config:app:set theming_customcss css --value='$(cat /tmp/custom-css.txt)'\"")
